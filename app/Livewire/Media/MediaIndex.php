...
class MediaIndex extends Component
{
    ...
    public function getClipsProperty()
    {
        ...
        if ($this->outletFilter) {
            $query->where('outlet_name', 'like', "%{$this->outletFilter}%");
        }
...
PROMPT;
            ...
            $aiResponse = \App\Support\AI\AnthropicClient::send([
                'max_tokens' => 1000,
                'messages' => [
            ...
            if (! ($aiResponse['error'] ?? false)) {
                $content = data_get($aiResponse, 'content.0.text');
