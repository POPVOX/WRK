        ...
        <section class="app-surface p-5">
            ...
            <form wire:submit="saveMessage" class="mt-4 space-y-4">
                <div>
                    ...
                    <input id="dry-run-subject" type="text" wire:model.defer="subject" class="mt-1 block w-full rounded-lg border-gray-300 text-sm" placeholder="A useful resource for @{{office}}">
                    @error('subject') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="dry-run-body" class="text-sm font-semibold text-gray-700">Plain-text message</label>
                    <textarea id="dry-run-body" wire:model.defer="bodyText" rows="10" class="mt-1 block w-full rounded-lg border-gray-300 text-sm" placeholder="Hi @{{first_name}},&#10;&#10;I wanted to share..."></textarea>
                    ...
                    <div class="grid gap-4 xl:grid-cols-[minmax(13rem,0.9fr)_minmax(19rem,1.2fr)_minmax(12rem,0.8fr)_auto] xl:items-center">
                        ...
                        <div>
                            @if($profileEmails->isNotEmpty())
                                <select wire:change="selectEmail({{ $recipient->id }}, $event.target.value)" class="block w-full rounded-lg border-gray-300 text-sm">
