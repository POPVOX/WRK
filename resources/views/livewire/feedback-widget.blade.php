<div x-data="{
    ...
    @if(!$isOpen)
        <button wire:click="open"
            class="group flex items-center justify-center w-10 h-10 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200"
            title="Feedback">
    ...
    @if($isOpen)
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden transition-all duration-300"
            style="width: {{ $isMinimized ? '280px' : '380px' }};" @paste="handlePaste($event)">
            {{-- Header --}}
            <div class="bg-gradient-to-r from-indigo-600 to-purple-600 px-4 py-3 flex items-center justify-between">
                ...
                <div class="flex items-center gap-1">
                    <button wire:click="{{ $isMinimized ? 'expand' : 'minimize' }}"
                ...
                <div class="p-4">
                    ...
                    @else
                        ...
                        <form wire:submit="submit" class="space-y-4">
                            {{-- Feedback Type --}}
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">What type of
                                    feedback?</label>
                                <div class="flex flex-wrap gap-2">
                                    @foreach(['bug' => '🐛 Bug', 'suggestion' => '💡 Suggestion', 'compliment' => '🎉 Compliment', 'question' => '❓ Question', 'general' => '💬 General'] as $type => $label)
                                        <button type="button" wire:click="$set('feedbackType', '{{ $type }}')"
                            ...
                            {{-- Category (optional) --}}
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Category
                                    (optional)</label>
                            ...
                            <div>
                                ...
                                <textarea wire:model="message" rows="4"
                                    ...
                                    class="w-full text-sm rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white placeholder-gray-400 focus:ring-indigo-500 focus:border-indigo-500 resize-none"></textarea>
                                @error('message')
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="file" wire:model="screenshot" accept="image/*" x-ref="screenshotInput"
                                        class="hidden" id="screenshot-input">
